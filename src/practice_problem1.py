...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        if len(self.contents + additional_contents) <= self.volume:
            ...
            new = self.volume - len(self.contents)
            for k in range (new):
                self.contents = self.contents +additional_contents[k]
            for k in range (new, len(additional_contents)):
                self.string = self.string + additional_contents[k]
        return self.string
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents)>=new_volume:
            ...
            for k in range (new_volume):
                self.contents = self.contents + a[k]
            for k in range(new_volume,len(a)):
                self.string = self.string + a[k]
    ...
    def steal(self, other_box):
        ...
        other_box.contents = self.string
